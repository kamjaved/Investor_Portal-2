...
export const addRation = (formData, history) => async (dispatch) => {
  try {
    ...
    const res1 = await axios.post(
      "https://donation-central.herokuapp.com/api/delivery",
      formData
    );
    dispatch({
...
export const editRation = (formData, history, id) => async (dispatch) => {
  try {
    ...
    const res1 = await axios.patch(
      `https://donation-central.herokuapp.com/api/delivery/${id}`,
      formData
    );
    dispatch({
      type: types.GET_RATION,
...
// Search delivery
export const searchDelivery = (formData, history, city, area) => async (
  dispatch
) => {
  try {
    const res = await axios.post(
      `https://donation-central.herokuapp.com/api/delivery/searchDelivery?city=${city}&area=${area}`,
      formData
    );
